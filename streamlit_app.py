...
def get_series(df: pd.DataFrame, col: str) -> pd.Series:
    s = df[col]
    # في بعض الحالات يكون DataFrame بعدة أعمدة -> نأخذ أول عمود
...
st.caption("أداة تعليمية لاختيار مرشحي شراء وشورت وفق VCP + Stage2/4. لا تعتبر توصية استثمارية.")
...
if st.button("تشغيل الفلتر الآن", type="primary"):
    ...
    with c1:
        ...
        if long_df.empty:
            st.write("لا يوجد مرشحين متوافقين مع القواعد الحالية.")
        else:
...
st.markdown(
    """
    **تنبيه:** راقب كل سهم يدويًا قبل التنفيذ. تحقق من الاختراق/الكسر، الفوليوم، ووقف الخسارة.
    """
)
